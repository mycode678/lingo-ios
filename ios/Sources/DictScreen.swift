...
struct DictScreen: View {
            ...
            ZStack(alignment: .bottom) {
                Group {
                    if store.entryHTML.isEmpty {
                        ...
                        VStack(spacing: 0) {
                            if !store.related.isEmpty { relatedChips }
                            entryWeb
                        }
                    }
                }
    ...
    private var relatedChips: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            ...
            .padding(.horizontal, T.side).padding(.vertical, 7)
        }
        .background(Color(.systemBackground))
    }

    private var entryWeb: some View {
    ...
    private var bodyTail: some View {
            EmptyView()
                .navigationTitle(store.word.isEmpty ? "查词" : store.word)
            .navigationBarTitleDisplayMode(.inline)
            .searchable(text: $q, prompt: "查一个词（中文也能查）")
